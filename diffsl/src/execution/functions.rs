...
pub extern "C" fn dsin(x: f64, dx: f64) -> f64 {
    x.cos() * dx
}
...
pub extern "C" fn dcos(x: f64, dx: f64) -> f64 {
    -x.sin() * dx
}
...
pub extern "C" fn dtan(x: f64, dx: f64) -> f64 {
...
pub extern "C" fn dexp(x: f64, dx: f64) -> f64 {
    x.exp() * dx
}
...
pub extern "C" fn dlog(x: f64, dx: f64) -> f64 {
    dx / x
}
...
pub extern "C" fn dlog10(x: f64, dx: f64) -> f64 {
    dx / (x * 10.0_f64.ln())
}
...
pub extern "C" fn dsqrt(x: f64, dx: f64) -> f64 {
    0.5 * dx / x.sqrt()
}
...
pub extern "C" fn dabs(x: f64, dx: f64) -> f64 {
    if x > 0.0 {
        dx
    } else {
        -dx
...
pub extern "C" fn dcopysign(_x: f64, dx: f64, y: f64, _dy: f64) -> f64 {
    dx.copysign(y)
}
...
pub extern "C" fn dpow(x: f64, dx: f64, y: f64, dy: f64) -> f64 {
    x.powf(y - 1.0) * (y * dx + x * dx.ln() * dy)
}

pub extern "C" fn min(x: f64, y: f64) -> f64 {
    x.min(y)
}

pub extern "C" fn dmin(x: f64, dx: f64, y: f64, dy: f64) -> f64 {
    if x < y {
        dx
    } else {
        dy
...
pub extern "C" fn max(x: f64, y: f64) -> f64 {
    x.max(y)
}

pub extern "C" fn dmax(x: f64, dx: f64, y: f64, dy: f64) -> f64 {
    if x > y {
        dx
    } else {
        dy
...
// (f'(x))/(2 cosh(f(x)) + 2)
pub extern "C" fn dsigmoid(x: f64, dx: f64) -> f64 {
    let cosh = x.cosh();
    dx / (2.0 * cosh + 2.0)
}

pub extern "C" fn arcsinh(x: f64) -> f64 {
    x.asinh()
}

// d/dx(sinh^(-1)(f(x))) = (f'(x))/sqrt(f(x)^2 + 1)
pub extern "C" fn darcsinh(x: f64, dx: f64) -> f64 {
    dx / (x.powi(2) + 1.0).sqrt()
}

pub extern "C" fn arccosh(x: f64) -> f64 {
    x.acosh()
}

// d/dx(cosh^(-1)(f(x))) = (f'(x))/(sqrt(f(x) - 1) sqrt(f(x) + 1))
pub extern "C" fn darccosh(x: f64, dx: f64) -> f64 {
    dx / ((x - 1.0).sqrt() * (x + 1.0).sqrt())
}

pub extern "C" fn heaviside(x: f64) -> f64 {
    if x >= 0.0 {
        1.0
    } else {
        0.0
...
pub extern "C" fn dheaviside(_x: f64, _dx: f64) -> f64 {
    0.0
}
...
// (f'(x))/(cosh^2(f(x)))
pub extern "C" fn dtanh(x: f64, dx: f64) -> f64 {
    let cosh = x.cosh();
    dx / cosh.powi(2)
}
...
pub extern "C" fn dsinh(x: f64, dx: f64) -> f64 {
    dx * x.cosh()
}
...
pub extern "C" fn dcosh(x: f64, dx: f64) -> f64 {
    dx * x.sinh()
}
