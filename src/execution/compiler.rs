...
use super::{
    codegen::{CalcOutFunc, ResidualFunc, StopFunc, U0Func},
...
impl Compiler {
    ...
    pub fn from_discrete_model(model: &DiscreteModel, out: &str) -> Result<Self> {
        ...
        CompilerTryBuilder {
            ...
            data_builder: |context| {
                ...
                let _residual_grad = codegen.compile_gradient(
                    _residual,
                    &[
    ...
    pub fn set_u0(&self, yy: &mut [f64], yp: &mut [f64], data: &mut [f64]) -> Result<()> {
        ...
        if yp.len() != number_of_states {
            return Err(anyhow!(
                "Expected {} state derivatives, got {}",
                number_of_states,
                yp.len()
            ));
        }
        self.with_data(|compiler| {
            ...
            unsafe {
                compiler.jit_functions.set_u0.call(data_ptr, yy_ptr, yp_ptr);
            }
        });
        Ok(())
    }
    ...
    pub fn set_u0_grad(
        ...
        if dyp.len() != number_of_states {
            return Err(anyhow!(
                "Expected {} state derivatives for dyp, got {}",
                number_of_states,
                dyp.len()
            ));
        }
        ...
        self.with_data(|compiler| {
            ...
            unsafe {
                compiler
                    .jit_grad_functions
                    .set_u0_grad
                    .call(data_ptr, ddata_ptr, yy_ptr, dyy_ptr, yp_ptr, dyp_ptr);
            }
        });
        Ok(())
    }
        ...
        self.with_data(|compiler| {
            ...
            unsafe {
                compiler
                    .jit_functions
                    .calc_stop
                    .call(t, yy_ptr, yp_ptr, data_ptr, stop_ptr);
            }
        });
        Ok(())
    }
        ...
        self.with_data(|compiler| {
            ...
            unsafe {
                compiler
                    .jit_functions
                    .residual
                    .call(t, yy_ptr, yp_ptr, data_ptr, rr_ptr);
            }
        });
        Ok(())
    }
    ...
    pub fn residual_grad(
        ...
        self.with_data(|compiler| {
            ...
            let ddata_ptr = ddata.as_mut_ptr();
            unsafe {
                compiler.jit_grad_functions.residual_grad.call(
                    t, yy_ptr, dyy_ptr, yp_ptr, dyp_ptr, data_ptr, ddata_ptr, rr_ptr, drr_ptr,
                );
            }
        });
        Ok(())
        ...
        self.with_data(|compiler| {
            ...
            unsafe {
                compiler
                    .jit_grad_functions
                    .calc_out_grad
                    .call(t, yy_ptr, dyy_ptr, yp_ptr, dyp_ptr, data_ptr, ddata_ptr);
            }
        });
        Ok(())
    }
...
mod tests {
    ...
    fn test_from_discrete_str() {
        ...
        compiler
            .set_u0(u0.as_mut_slice(), up0.as_mut_slice(), data.as_mut_slice())
            .unwrap();
    ...
    fn tensor_test_common(text: &str, tmp_loc: &str, tensor_name: &str) -> Vec<Vec<f64>> {
        ...
        compiler
            .set_inputs(inputs.as_slice(), data.as_mut_slice())
            .unwrap();
        compiler
            .set_u0(u0.as_mut_slice(), up0.as_mut_slice(), data.as_mut_slice())
            .unwrap();
        compiler
            ...
            .unwrap();
        compiler
            .calc_out(0., u0.as_slice(), up0.as_slice(), data.as_mut_slice())
            .unwrap();
        results.push(
            compiler
    ...
    fn test_additional_functions() {
        ...
        let mut up = vec![0., 0.];
        compiler
            .set_u0(u.as_mut_slice(), up.as_mut_slice(), data.as_mut_slice())
            .unwrap();
        ...
        compiler
            .calc_out(0., u.as_slice(), up.as_slice(), data.as_mut_slice())
            .unwrap();
